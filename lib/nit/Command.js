module.exports = function (nit, Self)
{
    return (Self = nit.defineClass ("nit.Command"))
        ...
        .staticMethod ("run", async function (...args)
        {
            let cls = this;
            let cmd = await new cls;

            return await this.runCommand (cmd, null, ...args);
        })
        .staticMethod ("runCommand", async function (cmd, ctx, ...args)
        {
            let cls = this;

            ctx = ctx || await new cls.Context;

            if (args.length)
            {
                let options = cls.Input.parseArgv (...args);

                ctx.input = new cls.Input (options);
            }
            ...
            try
            {
                try
                {
                    ...
                    return ctx.output = await cmd.run (ctx);
                }
                finally
                {
