module.exports = function (nit, Self)
{
    ...
    return (Self = nit.defineClass ("nit.Command"))
        ...
        .defineInnerClass ("completers.Command", "nit.compgen.Completer", Completer =>
        {
            Completer
        ...
        .defineInnerClass ("Input", Input =>
        {
            Input
                ...
                .staticMethod ("import", function (fields)
                {
                    fields.forEach (f =>
                    {
                        this.option (f.spec, f.type, f.description, nit.omit (f.toPojo (true), "get", "set"));
                    });
                ...
                .staticMethod ("parseArgv", function (argv)
                {
                    ...
                    while ((arg = argv.shift ()))
                    {
                        ...
                        if (subcommandOption)
                        {
                            options[subcommandOption.name] = { input: subcommandOption.class.lookup (arg).Input.parseArgv (argv.splice (0)) };
                        }
                        else
                        {
