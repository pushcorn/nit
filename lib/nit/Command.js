module.exports = function (nit, Self)
{
    ...
    return (Self = nit.defineClass ("nit.Command"))
        ...
        .plugin ("lifecycle-component", "run", "catch", "finally")
        ...
        .staticMethod ("buildPropertyDescription", function (p, defvals)
        {
            ...
            return p.description + ((p.required || p.array || p.type == "boolean" || nit.is.func (defval) || nit.is.empty (defval)) ? "" : "\n{ default: " + defval + " }");
        })
        ...
        .defineInnerClass ("Context", "nit.Context", function (Context)
        {
            Context
                ...
                .onConstruct (function ()
                {
                    if (!this.input)
                    {
                        let inputClass = this.constructor.outerClass.Input;
        ...
        .onConfigureQueueForRun (function (queue, command, args)
        {
            let cls = command.constructor;
            let ctx = args[0];

            ctx = ctx instanceof cls.Context ? ctx : cls.Context.new (cls.Input.fromArgv (...args));
            ctx.command = command;

            args.splice (0, args.length, ctx);

            queue
                .after ("run.checkResult", "run.castOutput", function ()
                {
                    if (cls.outputType)
                    {
                ...
                .failure (function (c)
                {
                    Object.setPrototypeOf (ctx.error = c.error, Error.prototype); // In test mode, native errors are not instances of Error.

                    c.error = undefined;

                    return command.catch (ctx);
                })
                .complete (function (c)
                {
                    ctx.error = c.error;
                    ...
                    return nit.Queue ()
                        .push (command.finally.bind (command, ctx))
                        .complete (function (c)
                        {
