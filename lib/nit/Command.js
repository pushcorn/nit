module.exports = function (nit, Self)
{
    return (Self = nit.defineClass ("nit.Command"))
        ...
        .categorize ()
        .defineInnerPlugin ("Plugin", Plugin =>
        {
            Plugin
                .categorize ()
                .method ("preRun", /* istanbul ignore next */ function (ctx, cmd) { nit.log ("ha run"); }) // eslint-disable-line no-unused-vars
                .method ("postRun", /* istanbul ignore next */ function (ct, cmdx) {}) // eslint-disable-line no-unused-vars
                .method ("preCatch", /* istanbul ignore next */ function (ctx, cmd) {}) // eslint-disable-line no-unused-vars
                .method ("postCatch", /* istanbul ignore next */ function (ctx, cmd) {}) // eslint-disable-line no-unused-vars
                .method ("preFinally", /* istanbul ignore next */ function (ctx, cmd) {}) // eslint-disable-line no-unused-vars
                .method ("postFinally", /* istanbul ignore next */ function (ctx, cmd) {}) // eslint-disable-line no-unused-vars
            ;
        })
        ...
        .staticMethod ("catch", function (c)
        {
            return this.staticMethod (Self.kCatch, c);
        })
        .staticMethod ("finally", function (f)
        {
            return this.staticMethod (Self.kFinally, f);
        })
        .method ("run", async function (...options)
        {
            let cmd = this;
            ...
            try
            {
                try
                {
                    await cls.applyPlugins ("plugins", "preRun", ctx, cmd);

                    let run = cls[Self.kRun];

                    if (run)
                    {
                        ctx.output = await run.call (cmd, ctx);
                    }
                ...
                {
                    await cls.applyPlugins ("plugins", "postRun", ctx, cmd);
                }
            }
            catch (e)
            {
                ...
                try
                {
                    await cls.applyPlugins ("plugins", "preCatch", ctx, cmd);
                    await cmd.catch (ctx);
                }
                finally
                {
                    await cls.applyPlugins ("plugins", "postCatch", ctx, cmd);
                }
            }
            finally
            {
                try
                {
                    await cls.applyPlugins ("plugins", "preFinally", ctx, cmd);
                    await cmd.finally (ctx);
                }
                finally
                {
                    await cls.applyPlugins ("plugins", "postFinally", ctx, cmd);
                }
            }
        })
        ...
        .method ("finally", function (ctx)
        {
            let f = this.constructor[Self.kFinally];
