module.exports = function (nit, Self)
{
    ...
    return (Self = nit.defineClass ("nit.Command"))
        ...
        .k ("context", "initContext", "destroyContext", "castOutput", "setErrorPrototype")
        ...
        .defineInnerClass ("Option", "nit.Model.Field", function (Option)
        {
            Option
                ...
                .property ("kvp", "boolean|string") // Add .kvp getter to the array option
                ...
                .onPostConstruct (function ()
                {
                    var option = this;
                    var kvp = option.kvp;
                    var __set = option.set;

                    kvp = kvp === true ? "=" : kvp;

                    option.set = function (v, ctx)
                    {
                        var owner = this;

                        __set.call (owner, v, ctx);

                        if (kvp && option.array)
                        {
                            v = owner[option.name];

                            nit.dpg (v, "kvp", function () { return nit.parseKvp (this, kvp); }, true, false);
                        }
                    };

                    nit.dpv (option.set, "__set", __set, true, false);
                })
            ;
        })
        .defineInnerClass ("Input", "nit.Model", Input =>
        {
            Input
                .m ("error.model_validation_failed", "One or more options are invalid:%{#validationContext.violations}\n    - %{field \\|\\| constraint}: %{message}%{/}")
        ...
        .defineInnerClass ("Context", "nit.Context", function (Context)
        {
            Context
                .field ("[input]", Self.Input.name, "The command input.",
                {
                    ...
                    {
                        let owner = this;
                        let inputClass = owner.constructor.outerClass.Input;

                        if (!(value instanceof inputClass))
                        {
                            value = new inputClass (value);
                        }

                        return value;
                    }
                    ,
                    defval: function (prop, owner)
                    {
                        let inputClass = owner.constructor.outerClass.Input;

                        if (inputClass && !inputClass.fields.some (f => f.required))
                        {
                            return {};
                        }
                    }
                ...
                .method ("validateInput", async function ()
                {
                    let self = this;
                    let cls = self.constructor;
                    let inputClass = cls.outerClass.Input;

                    return await inputClass.validate (self.input);
                })
                .onLookupServiceProvider (function (type)
                {
                    return this.command?.lookupServiceProvider (type);
                })
            ;
        })
        ...
        {
            Method.chains.run.after (method + ".invokeHook", method + ".checkResult", function (cmd, ctx)
            {
        ...
        {
            Method
                .before (Self.kInitContext, async function (cmd)
                {
                    ...
                    if (args.length || !ctx.input)
                    {
                        ctx.input = cls.Input.fromArgv (...args);
                    }

                    await ctx.validateInput ();
                })
                .after ("run.checkResult", Self.kCastOutput, function (cmd, ctx)
                {
                    let cls = cmd.constructor;
                ...
                .beforeFailure (Self.kSetErrorPrototype, function (cmd, ctx)
                {
                    Object.setPrototypeOf (ctx.error = this.error, Error.prototype); // In test mode, native errors are not instances of Error.
                    this.error = null;
                })
                .afterComplete (Self.kDestroyContext, function (cmd, ctx)
                {
