    ...
    return (Self = nit.defineClass ("nit.CompgenCompleter"))
        ...
        .defineCompleter ("OptionCompleter", OptionCompleter =>
        {
            OptionCompleter
                ...
                .field ("[conditions]", "object", "Other completion conditions.")
                .onApplicableTo (function (ctx)
                {
                    let { path, conditions } = this;
                    let [className, optName] = nit.kvSplit (path, ".", true);

                    return ctx.commandClass?.name == className
                        && ctx.currentOption?.name == optName
                        && (nit.is.empty (conditions)
                            || nit.is.equal (conditions, nit.pick (ctx.specifiedValues, nit.keys (conditions))))
                    ;
                })
            ;
        })
        ...
        .staticTypedMethod ("completeForOption",
            {
                path: "string", generator: "function", conditions: "object"
            },
            function (path, generator, conditions)
            {
                this.completers.push (new Self.OptionCompleter (generator, path, { conditions }));
