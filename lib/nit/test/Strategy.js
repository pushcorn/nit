module.exports = function (nit, Strategy)
{
    ...
    return (Strategy = nit.defineClass ("nit.test.Strategy"))
        ...
        .method ("withApp", function (name, root)
        {
            this.app = new Strategy.App (name, root);
        ...
        .method ("expectingMethodToReturnValue", function (path, retval, ...args)
        {
            ...
            {
                return Strategy.invoke (strategy, path, args);
            });
        ...
        {
            let message = nit.format ("the method %{path} () to return a value of type '%{type}' when invoked with (%{args.join (', ')})", { path, args, type });
            let expector = new Strategy.Expector (message, new Strategy.TypeValidator ({ expected: type }), function (strategy)
            {
                return Strategy.invoke (strategy, path, args);
            });
        ...
        {
            let message = nit.format ("the method %{path} () to throw '%{error}' when invoked with (%{args.join (', ')})", { path, args, error });
            let expector = new Strategy.Expector (message, new Strategy.ErrorValidator ({ expected: error }), function (strategy)
            {
                return Strategy.invoke (strategy, path, args);
            });
