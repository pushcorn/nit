module.exports = function (nit, Self)
{
    ...
    return (Self = nit.defineClass ("nit.test.Strategy"))
        ...
        .field ("initializers...", "function", "Tasks to run before test starts.")
        ...
        .defineInnerClass ("ErrorValidator", "nit.test.Strategy.Validator", ErrorValidator =>
        {
            ErrorValidator
                ...
                .field ("expected", "any", "The result type.")
                    .constraint ("type", "RegExp", "string", "function")
                .onValidate (function (strategy, value)
                {
                    if (arguments.length == 1 && !strategy.error)
                    {
                ...
                .method ("apply", function (strategy)
                {
                    ...
                    self.target[method] = function (...args)
                    {
                        return nit.Queue ()
                            .push (function ()
                            {
                                return self.targetMethod.apply (self.target, args);
                            })
                            .complete (function (ctx)
                            {
                                if (ctx.error)
        ...
        {
            this.initializers.push (Self.trackCallback (cb));
        ...
        {
            let message = Self.render ("the property '%{property}' to be of type '%{type|formatType}'", { property, type });
        ...
        .method ("commit", function ()
        {
            ...
            for (let init of self.initializers)
            {
                init.call (self, self);
            }
