module.exports = function (nit, Self)
{
    return (Self = nit.defineClass ("nit.Compgen"))
        ...
        .constant ("ACTIONS", nit.index (["COMMAND", "OPTION", "VALUE", "NONE", "FILE", "DIR"]))
        ...
        .defineInnerClass ("Context", function (Context)
        {
            Context
                ...
                {
                    return nit.find (this.commandClass?.Input.getPositionalOptions (), o => o.type != "boolean" && (o.array || !this.specifiedOptions.includes (o.flag)));
                })
            ;
        })
        ...
        {
            return nit.lookupComponents ("completers", Self.Completer)
                ...
                if ((dashes = word.match (/^(--?)([^-].*)?/)))
                {
                    ...
                    if (flag)
                    {
                        ...
                        if (dashes.length == 1)
                        {
                            for (let ch of flag.split (""))
                            {
                                if ((opt = ctx.commandClass?.Input.getOptionByShortFlag (ch)) && !opt.array)
                                {
                        ...
                        if ((opt = ctx.commandClass?.Input.getOptionByFlag (flag)) && !opt.array)
                        {
                            ctx.specifiedOptions.push (opt.flag);
                        }
                    }
            ...
            switch (ctx.state)
            {
                ...
                case STATES.option:
                    ...
                    if (ctx.commandClass)
                    {
                        ...
                        {
                            for (let o of ctx.commandClass.Input.properties)
                            {
                        ...
                        else
                        {
                            ...
                            if ((opt = ctx.commandClass.Input.properties[0]))
                            {
