module.exports = function (nit)
{
    return nit.defineCommand ("commands.Lint")
        ...
        .method ("run", async function (ctx)
        {
            ...
            let currentRc, targetRc, rcCopied;

            if (cwd != pwd
                && (currentRc = nit.lint.EsLint.findRc ())
                && !(targetRc = nit.lint.EsLint.findRc (cwd)))
            {
                targetRc = nit.new ("nit.File", nit.path.join (cwd, currentRc));
                currentRc = nit.new ("nit.File", nit.path.join (pwd, currentRc));
                currentRc.copy (targetRc.path);
                rcCopied = true;
            }
