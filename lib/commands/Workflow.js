            ...
            Subcommand
                .onBuildSubcommand ((Subcommand, Workflow, descriptor) =>
                {
                    Workflow = Workflow || nit.defineWorkflow (nit.ComponentDescriptor.toClassName (descriptor.name, descriptor.category))
                        .configure (nit.require (descriptor.path))
                    ;

                    Subcommand
                        .describe (Workflow.config.description)
                        .defineInput (Input =>
                        {
                            Input.import (nit.array (Workflow.config.options));
                        })
                    ;
                })
                .staticMethod ("lookup", function (name)
                {
                    ...
                    catch (e)
                    {
                        ...
                        {
                            cls.throw ("error.workflow_not_found", { name });
                        }
                    }
                })
