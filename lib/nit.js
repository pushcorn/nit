        ...
        use.parsers.push (
            function requireModule (property, name)
            {
                if (name[0] == "*")
                {
                    ...
                    return nit.dpv (() => nit.requireModule (name), "name", property);
                }
            }
            ,
...
nit.dpgs (nit,
{
    ...
    DEFAULT_CLASS_SUBDIRS: "lib public",
...
nit.dpvs (nit,
{
    ...
    handleException: function (err)
    {
        ...
        nit.log.e (nit.DEBUG ? err.stack : err.message);
    }
    ,
    dbg: function ()
    {
    ...
    require: function (path, optional)
    {
        ...
        if (file != path && path.match (nit.CLASS_NAME_PATTERN))
        {
    ...
    ready: function ()
    {
        ...
        return READY ? READY_QUEUE.run (() => nit) : nit;
    }
    ,
    ...
    lookupCommand: function (name)
    {
        ...
        {
            let isCommand = e.context?.data?.component == name;

            if (isCommand && e.code == "error.component_not_found")
            {
                nit.throw ("error.command_not_found", { command: name });
            }
            else
            if (isCommand && e.code == "error.invalid_component")
            {
