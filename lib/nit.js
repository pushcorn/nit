...
nit.dpgs (nit,
{
    ...
    ,
    COMMANDS_SUBDIR: nit.memoize (function ()
    {
        return nit.ENV.NIT_COMMANDS_SUBDIR || nit.DEFAULT_COMMANDS_SUBDIR;
    })
    ,
...
nit
    .defineClass ("nit.ComponentDescriptor")
    .field ("<path>", "string", "The component path.")
    ...
    {
        let na = this.name.split (":");
        let nb = that.name.split (":");
...
nit.dpvs (nit,
{
    getComponentDescriptor: function (className)
    {
    ...
    listComponentPaths: function (subdir)
    {
        ...
        let lastSubdir = subdir.split (nit.path.sep).pop ();
        let subdirPath = nit.path.sep + subdir;
        ...
        while (dirs.length)
        {
            ...
            for (let f of nit.fs.readdirSync (dir.path, { withFileTypes: true }))
            {
                ...
                if (nit.isDir (p))
                {
                    let relPath = p.slice (dir.root.length);

                    if (f.name == lastSubdir && relPath.endsWith (subdirPath))
                    {
                        paths.push (
                        {
                            path: p,
                            classNamespace: relPath.slice (1).replaceAll (nit.path.sep, "."),
    ...
    ,
    listComponents: function (subdir, namesOnly)
    {
        let components = [];
        ...
        for (let p of nit.listComponentPaths (subdir))
        {
            for (let c of nit.fs.readdirSync (p.path))
            {
                ...
                components.push (nit.ComponentDescriptor (
                {
        ...
        return namesOnly ? components.map (c => c.name) : components;
    }
    ,
    lookupComponents: function (subdir, superclass)
    {
        superclass = nit.is.func (superclass) ? superclass : nit.lookupClass (superclass);

        return nit
            .listComponents (subdir)
            .map (c => nit.require (c.path))
            .filter (cls => nit.is.subclassOf (cls, superclass))
        ;
    }
    ,
    lookupComponent: function (subdir, name, superclass)
    {
        let component = nit.find (nit.listComponents (subdir), (c) => c.name == name || c.className == name);
        ...
        let cls = nit.require (component.path);

        if (superclass && !nit.is.subclassOf (cls, nit.lookupClass (superclass)))
        {
            nit.throw ("error.invalid_component", { component: name, superclass });
        }

        return cls;
    }
    ,
    listCommands: function (namesOnly)
    {
        return nit.listComponents (nit.COMMANDS_SUBDIR, namesOnly);
    }
    ,
    lookupCommand: function (name)
    {
        try
        {
            return nit.lookupComponent (nit.COMMANDS_SUBDIR, name, "nit.Command");
        }
        catch (e)
        {
