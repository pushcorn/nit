...
nit.Object
    .do ("ClassTypeParser", ClassTypeParser =>
    {
        ClassTypeParser.prototype.supports = nit.do (ClassTypeParser.prototype.supports, function (supports)
        {
            return function (type, nullable)
            {
                if (nit.COMPLETION_MODE)
                {
                    return !!(type && type.match (nit.CLASS_NAME_PATTERN));
                }
                else
                {
                    return supports.call (this, type, nullable);
                }
            };
        });
    })
    .do ("ConfigTypeParser", ConfigTypeParser =>
    {
        ConfigTypeParser.prototype.cast = function (config)
        {
            if (nit.is.str (config))
            {
                let file = nit.resolveAsset (config);

                return file ? nit.require (file) : nit.config (config);
            }
            else
...
nit.dpvs (nit,
{
    ...
    handleException: function (err)
    {
        ...
        nit.log.e (nit.find ([err.message, err], nit.is.not.empty));
        nit.debug ("nit", err.stack);
    }
    ,
    shutdown: function ()
    {
        SHUTDOWN_TASKS.push (...arguments);

        return SHUTDOWN ? nit.invoke.each (SHUTDOWN_TASKS, t => nit.ns.invoke (t), nit) : nit;
    }
    ,
    ...
    ,
    resetRequireCache: function (...classesToDelete)
    {
        REQUIRE_CACHE = {};

        nit.each (nit.keys (nit.CLASSES), function (name)
        {
            let del = classesToDelete.some (test =>
            {
                if (nit.is.func (test))
                {
                    return test (name);
    ...
    ,
    loadClass: nit.do (nit.loadClass, function (loadClass)
    {
        return function (name)
        {
            return loadClass (name) || nit.require (name, true);
        };
    })
    ,
    ...
    loadConfigs: function ()
    {
        ...
        return nit.invoke.each (entries, e => nit.config (e.k, e.v));
    }
    ,
    ...
    listComponents: nit.do (nit.listComponents, function (pListComponents)
    {
        return function (category, returnNames)
        {
            ...
            for (let p of nit.listComponentPaths (category))
            {
                ...
                {
                    let className = p.classNamespace + "." + nit.pascalCase (nit.path.parse (c).name);
    ...
    runCommand: async function ()
    {
        ...
        let ctx = await nit.Command.exec (name, args);
        let { exitCode, output } = ctx;

        process.exitCode = exitCode;

        if (!nit.is.empty (output))
        {
            nit.inspect (nit.toPojo (output));
        }
