...
nit.dpgs (nit,
{
    ...
    {
        return nit.arrayUnique ([nit.NIT_HOME, nit.resolvePackageRoot ()]
            .concat (nit.ENV.NIT_PROJECT_PATHS && nit.ENV.NIT_PROJECT_PATHS.split (":") || []))
            .concat (nit.USER_HOME)
        ;
    })
    ,
    ...
    ASSET_PATHS: nit.memoize (function ()
    {
        ...
        for (let p of nit.PROJECT_PATHS.reverse ())
        {
            paths.push (p);
...
nit.dpvs (nit,
{
            ...
            for (let f of nit.fs.readdirSync (dir.path, { withFileTypes: true }))
            {
                ...
                if (nit.isDir (p))
                {
                    if (f.name == subdir)
                    {
                        ...
                        {
                            path: p,
                            className: p.slice (dir.root.length + 1).replaceAll (nit.path.sep, ".").replace (/\.js$/, ""),
    ...
    listComponents: function (subdir)
    {
        ...
        for (let p of nit.listComponentPaths (subdir))
        {
            ...
            {
                let name = nit.kababCase (nit.path.parse (c).name);
