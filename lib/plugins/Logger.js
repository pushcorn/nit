        ...
        .defineInnerClass ("Mixin", Mixin =>
        {
            Mixin
                ...
                .method ("log", function (message, ...args)
                {
                    ...
                    return host.writeLog (logger.prefix + logger.formatMessage (host, message, ...args) + logger.stack);
                })
                ...
                .do (function ()
                {
                    nit.log.LEVELS.forEach (level =>
                    {
                        this.method (level, function (message, ...args)
                        {
                            ...
                            if (logger.colorEnabled && !loggerPrefix && color)
                            {
                                prefix = Self.Colorizer[color] (prefix);
                            }

                            return host.writeLog (prefix + message + logger.stack);
                        });
                    });
                })
            ;
