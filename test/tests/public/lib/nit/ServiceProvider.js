test.method ("nit.ServiceProvider", "provides")
    .should ("return true if the type is supported")
        ...
        .given ("test.Db")
        .returns (true)
        .expectingMethodToThrow ("object.provides", null, "error.class_not_defined")
        .expectingMethodToReturnValue ("object.provides", "test.Db", true)
        .expectingMethodToThrow ("object.provides", "test.Srv", "error.class_not_defined")
        .commit ()
;


test.method ("nit.ServiceProvider", "nit.Object.caster", true)
    .can ("cast a function to a service provider")
        .up (s => s.Db = nit.defineClass ("test.Db"))
        .up (s => s.args = s.Db)
        .returnsInstanceOf ("test.DbProvider")
        .commit ()

    .can ("cast an object to a service provider")
        .up (s => s.Db = nit.defineClass ("test.Db"))
        .up (s => s.args = new s.Db)
        .returnsInstanceOf ("test.DbProvider")
        .commit ()

    .can ("cast a component pojo to a service provider")
        .up (s => s.Db = nit.defineServiceProvider ("test.serviceproviders.Db"))
        .up (s => s.MyClass = nit.defineClass ("MyClass")
            .field ("serviceproviders...", "nit.ServiceProvider")
        )
        .up (s => s.args = [{ "@name": "test:db" }, s.MyClass.fieldMap.serviceproviders])
        .returnsInstanceOf ("test.serviceproviders.Db")
        .commit ()

    .can ("cast a component string to a service provider")
        .up (s => s.Db = nit.defineServiceProvider ("test.serviceproviders.Db"))
        .up (s => s.MyClass = nit.defineClass ("MyClass")
            .field ("serviceproviders...", "nit.ServiceProvider")
        )
        .up (s => s.args = ["test:db", s.MyClass.fieldMap.serviceproviders])
        .returnsInstanceOf ("test.serviceproviders.Db")
        .commit ()

    .should ("return the value if the type cannot be casted")
        .up (s => s.args = 20)
        .returns (20)
        .commit ()
;
...
test.plugin ("nit.ServiceProvider", "lookupServiceProvider", { registerPlugin: true, instancePluginAllowed: true })
    ...
    .should ("return undefined if the class was not defined and required is not true")
        .given ("MyDb3")
        .returns ()
        .commit ()
